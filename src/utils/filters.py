...
import pandas as pd
def apply_filters(df: pd.DataFrame, branch=None, dates=None, channels=None, categories=None, products=None, semesters=None) -> pd.DataFrame:
    d=df.copy()
    ...
    if semesters and "Todos" not in semesters and "Semestre" in d: d=d[d.Semestre.isin(semesters)]
    return d
